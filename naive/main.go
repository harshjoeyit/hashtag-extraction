...
func UpdateWorker(postMsgs <-chan amqp.Delivery, workerID int) {
	for m := range postMsgs {
		...
		// Update the counts in DB
		for _, h := range post.ExtractHashtags() {
			err := db.Inc(h, 1)
			if err != nil {
				log.Printf("Failed to update db for hashtag: %s, error: %v", h, err)
			}
		}
